...
class LaSOTDataset(BaseDataset):
    """
    Modified LaSOT dataset for your specific structure
    Your dataset structure: lasot/001/color/*.png, lasot/002/color/*.png, etc.
    """
    ...
    def _construct_sequence(self, sequence_name):
        ...
        # 加载遮挡和出视野信息（如果文件存在）
        if os.path.exists(occlusion_label_path):
            full_occlusion = load_text(str(occlusion_label_path), delimiter=',', dtype=np.float64, backend='numpy')
        else:
            full_occlusion = np.zeros((ground_truth_rect.shape[0], 1))

        if os.path.exists(out_of_view_label_path):
            out_of_view = load_text(str(out_of_view_label_path), delimiter=',', dtype=np.float64, backend='numpy')
        else:
            out_of_view = np.zeros((ground_truth_rect.shape[0], 1))

        target_visible = np.logical_and(full_occlusion == 0, out_of_view == 0)

        # 构建图片路径 - 指向color文件夹中的png图片
        frames_path = '{}/{}/color'.format(self.base_path, sequence_name)

        # 获取color文件夹中所有png图片并排序
        if os.path.exists(frames_path):
            frame_files = sorted([f for f in os.listdir(frames_path) if f.endswith('.png')])
            frames_list = ['{}/{}'.format(frames_path, f) for f in frame_files]
        else:
            # 如果无法获取文件列表，使用数字编号方式
            frames_list = ['{}/{:08d}.png'.format(frames_path, frame_number)
                           for frame_number in range(1, ground_truth_rect.shape[0] + 1)]

        return Sequence(sequence_name, frames_list, 'lasot', ground_truth_rect.reshape(-1, 4),
                        object_class=sequence_name, target_visible=target_visible)
    ...
    def _get_sequence_list(self):
        # 自动生成001到050的序列列表
        sequence_list = ['{:03d}'.format(i) for i in range(1, 51)]
        return sequence_list